...
class WeaponDetector:
    def __init__(self, weights_path: str | None = None, conf: float = 0.30, imgsz: int = 640) -> None:
    ...
    def _load(self) -> None:
        if not Path(self.weights_path).exists():
            logger.warning("Weapon detector weights not found: %s", self.weights_path)
            return
        ...
        try:
            ...
            logger.info("Weapon detector loaded: %s", self.weights_path)
        except Exception:
            logger.exception("Failed to load weapon detector from %s", self.weights_path)
    ...
    def detect_image(self, frame) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("Weapon detection failed")
