...
if os.environ.get("ENABLE_ANTI_SPOOF", "1").strip().lower() in {
    "1", "true", "yes", "on"
}:
    try:
        ...
        # The server still starts normally when DeepFace is not installed.
        # The model badge remains missing until the dependency is installed.
        logging.getLogger(__name__).warning(
            "DeepFace anti-spoofing unavailable: %s",
            exc,
        )
...
ID_CONF = env_float("ID_CONF", 0.35)
ID_HOLD_SECONDS = env_float("ID_HOLD_SECONDS", 0.70)
...
ANTI_SPOOF_MIN_FACE_AREA = env_int("ANTI_SPOOF_MIN_FACE_AREA", 2500)
ANTI_SPOOF_MAX_FACES = env_int("ANTI_SPOOF_MAX_FACES", 5)
ANTI_SPOOF_HOLD_SECONDS = env_float("ANTI_SPOOF_HOLD_SECONDS", 1.5)
...
ID_RATIO_MIN = env_float("ID_RATIO_MIN", 0.85)
ID_RATIO_MAX = env_float("ID_RATIO_MAX", 2.80)
ID_AREA_MAX_RATIO = env_float("ID_AREA_MAX_RATIO", 0.32)
ID_MAX_WIDTH_RATIO = env_float("ID_MAX_WIDTH_RATIO", 0.88)
ID_MAX_HEIGHT_RATIO = env_float("ID_MAX_HEIGHT_RATIO", 0.72)
ID_MIN_RECTANGULARITY = env_float("ID_MIN_RECTANGULARITY", 0.30)
ID_MIN_EDGE_DENSITY = env_float("ID_MIN_EDGE_DENSITY", 0.015)
ID_MAX_EDGE_DENSITY = env_float("ID_MAX_EDGE_DENSITY", 0.45)
...
def find_id_card_contour(frame) -> dict[str, Any] | None:
    ...
    for contour in contours:
        ...
        if not (0.004 <= area_ratio <= ID_AREA_MAX_RATIO):
            continue
        ...
        # Real cards are usually bright and contain many text/photo edges.
        if bright_ratio < 0.28:
            continue

        if not (0.025 <= edge_density <= 0.40):
            continue
        ...
        score = (
            rectangularity * 0.25
            + min(bright_ratio / 0.70, 1.0) * 0.25
            + min(edge_density / 0.14, 1.0) * 0.20
            + ratio_score * 0.15
            + size_score * 0.10
            + center_score * 0.05
        )
...
def run_id_card_model(frame, model) -> tuple[int, list[dict[str, Any]]]:
    """Detect one ID card and avoid body/hand false boxes.

    The contour result is preferred because it surrounds the physical card.
    YOLO is retained as a secondary source when no reliable rectangle exists.
    """
    detections: list[dict[str, Any]] = []
    frame_height, frame_width = frame.shape[:2]
    ...
    # Prefer the actual rectangular card contour over loose YOLO body boxes.
    if contour_detection is not None:
        contour_box = contour_detection.get("box", [])

        if detections and len(contour_box) == 4:
            best_yolo = max(
                detections,
                key=lambda item: float(item.get("conf", 0.0)),
            )
            ...
            if len(yolo_box) == 4:
                overlap = iou(contour_box, yolo_box)
                if overlap >= 0.08:
                    combined_conf = max(
                        float(contour_detection.get("conf", 0.0)),
                        float(best_yolo.get("conf", 0.0)),
                    )
                    contour_detection["conf"] = combined_conf
                    contour_detection["label"] = (
                        f"ID-CARD {combined_conf:.0%}"
                    )

        return 1, [contour_detection]

    # If contour detection fails, retain only the strongest validated YOLO box.
    if detections:
        best = max(
            detections,
            key=lambda item: float(item.get("conf", 0.0)),
...
def run_anti_spoof(frame) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Detect faces and classify each one as live or spoof."""
    ...
    # DeepFace.extract_faces() does not support a max_faces argument in
    # several released versions. Request all detected faces, then apply the
    # configured limit locally for version compatibility.
    ...
    face_objects = face_objects[:ANTI_SPOOF_MAX_FACES]
    ...
    for face_object in face_objects or []:
        ...
        # With enforce_detection=False, some detector backends can return the
        # complete image as a fallback. Reject those low-confidence regions.
        if area < ANTI_SPOOF_MIN_FACE_AREA:
            continue

        if detector_confidence <= 0.0 and area > frame_width * frame_height * 0.70:
            continue
        ...
        color = (0, 190, 80) if is_real else (0, 0, 255)
    ...
    spoof_faces = [
        item for item in valid_faces if not bool(item.get("is_real", False))
    ]
    live_faces = [
        item for item in valid_faces if bool(item.get("is_real", False))
    ]

    if spoof_faces:
        best_score = max(float(item.get("score", 0.0)) for item in spoof_faces)
        label = "spoof"
        spoof_detected = True
    else:
        best_score = max(float(item.get("score", 0.0)) for item in live_faces)
        label = "live"
        spoof_detected = False

    return {
        "faces": len(valid_faces),
        "faces_live": len(live_faces),
        "anti_spoof": label,
...
def anti_spoof_thread(model_status: dict[str, bool]) -> None:
    ...
    if not DEEPFACE_AVAILABLE:
        with anti_spoof_lock:
            anti_spoof_meta = {
                ...
                "anti_spoof_conf": 0.0,
                "spoof_detected": False,
                "anti_spoof_error": "Install DeepFace",
            }
            anti_spoof_dets = []
        return
    ...
    # Let camera capture, JPEG encoding, and /healthz become ready before the
    # first TensorFlow/DeepFace inference starts.
...
def main() -> None:
    ...
    model_status = {
        ...
        "roboflow_weapon": ROBOFLOW_ENABLED,
        "face": ENABLE_ANTI_SPOOF and DEEPFACE_AVAILABLE,
        "anti_spoof": ENABLE_ANTI_SPOOF and DEEPFACE_AVAILABLE,
    }
    log.info("Models: %s", model_status)
    log.info(
        "Anti-spoof: enabled=%s available=%s detector=%s interval=%.1fs",
        ENABLE_ANTI_SPOOF,
        DEEPFACE_AVAILABLE,
        ANTI_SPOOF_DETECTOR,
        ANTI_SPOOF_INTERVAL,
