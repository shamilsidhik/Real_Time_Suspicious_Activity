...
ID_MODEL_EVERY = env_int("ID_MODEL_EVERY", 2)

# Strict confidence and multi-frame confirmation prevent background objects
# from being counted as ID cards.
ID_CONF = env_float("ID_CONF", 0.70)
ID_HOLD_SECONDS = env_float("ID_HOLD_SECONDS", 0.35)
ID_CONFIRM_HITS = env_int("ID_CONFIRM_HITS", 4)

# Generic contour detection mistakes windows, phones, paper, and screens for
# cards. Keep it disabled unless explicitly enabled for testing.
ID_CONTOUR_FALLBACK = env_bool("ID_CONTOUR_FALLBACK", False)
...
# -----------------------------------------------------------------------------
ID_AREA_MIN = env_int("ID_AREA_MIN", 1200)

# A real ID card is normally a landscape rectangle. These strict limits
# reject windows, monitors, paper sheets, phone screens, and body regions.
ID_RATIO_MIN = env_float("ID_RATIO_MIN", 1.15)
ID_RATIO_MAX = env_float("ID_RATIO_MAX", 2.20)
ID_AREA_MAX_RATIO = env_float("ID_AREA_MAX_RATIO", 0.18)
ID_MAX_WIDTH_RATIO = env_float("ID_MAX_WIDTH_RATIO", 0.65)
ID_MAX_HEIGHT_RATIO = env_float("ID_MAX_HEIGHT_RATIO", 0.42)
ID_MIN_RECTANGULARITY = env_float("ID_MIN_RECTANGULARITY", 0.50)
ID_MIN_EDGE_DENSITY = env_float("ID_MIN_EDGE_DENSITY", 0.025)
ID_MAX_EDGE_DENSITY = env_float("ID_MAX_EDGE_DENSITY", 0.35)
ID_MIN_BRIGHT_RATIO = env_float("ID_MIN_BRIGHT_RATIO", 0.18)
...
def run_id_card_model(frame, model) -> tuple[int, list[dict[str, Any]]]:
    """Detect an ID card using the trained model and strict geometry checks.

    Contour fallback is disabled by default because generic rectangles in the
    room are not evidence that an ID card is present.
    """
    detections: list[dict[str, Any]] = []
    frame_height, frame_width = frame.shape[:2]
    ...
    # A contour is accepted only when a validated YOLO card overlaps it.
    # A contour alone is never enough to claim an ID card.
    if contour_detection is not None and detections:
        ...
        if (
            len(contour_box) == 4
            and len(yolo_box) == 4
            and iou(contour_box, yolo_box) >= 0.35
        ):
            combined_conf = max(
    ...
    # Retain only the strongest strictly validated YOLO box.
    if detections:
        best = max(
            detections,
            key=lambda item: float(item.get("conf", 0.0)),
