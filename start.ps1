...
try {
    # Waitress writes normal startup messages to stderr. Run it through
    # cmd.exe so stdout and stderr are merged before PowerShell receives them.
    # This prevents NativeCommandError from stopping the script.
    $djangoCommand = (
        '"{0}" run_server.py 2>&1' -f
        $VENV_PY
    )
    ...
    if (
        $null -ne $djangoExitCode -and
        $djangoExitCode -ne 0
    ) {
        Write-Host ""
        Write-Host (
            "Django exited with code {0}." -f
            $djangoExitCode
        ) -ForegroundColor Red
