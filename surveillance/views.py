...
from surveillance.video_analyzer import queue_media_analysis
...
def _set_camera_detection(action):
    """Tell the camera server to pause/resume AI without closing the camera."""

    if action not in {"start", "stop"}:
        return {
            "ok": False,
            "error": "Invalid detection action",
        }
    ...
    return {
        "ok": False,
        "error": last_error
        or "Unable to control camera detection",
    }
...
def _create_detection_log(
    *,
    ...
    """
    Create one detection log, save the camera image, and queue an email alert
    for suspicious event types.
    """
    ...
    try:
        ...
        if _is_new_live_event(
            ...
            _create_detection_log(
                ...
                confidence_score=1.0,
                message=(
                    "Unknown person detected by live face recognition. "
                    f"Unknown faces visible: {unknown_face_count}."
                ),
            )
...
def home(request):
    """Display the public SecureVision landing page."""
...
def detection_api(request):
    ...
    if data.get("detection_enabled", True):
        _record_live_events(
            request,
            data,
        )
...
# ---------------------------------------------------------------------------
# Detection start/stop API
# ---------------------------------------------------------------------------

@login_required
@require_POST
def detection_control(request):
    action = str(
        request.POST.get("action", "")
    ).strip().lower()

    result = _set_camera_detection(action)
...
def upload(request):
    """
    Upload a video and start background AI analysis.
        ...
        try:
            ...
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    uploaded_video.status = "Processing"
                    uploaded_video.save()

                    try:
                        queue_media_analysis(uploaded_video.pk)
                    except Exception as analysis_error:
                        logger.exception(
                            "Could not queue video %s for analysis: %s",
                            uploaded_video.pk,
                            analysis_error,
                        )

                        uploaded_video.status = "Failed"
                        uploaded_video.save(update_fields=["status"])

                        messages.error(
                            request,
                            "The video was uploaded, but analysis could not start.",
                        )
                    else:
                        messages.success(
                            request,
                            "Image/video uploaded. AI analysis has started.",
                        )
            ...
            logger.exception("Upload page failed: %s", exception)
            form = None
            error = str(exception)
