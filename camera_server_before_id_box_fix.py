...
ID_CONF = env_float("ID_CONF", 0.25)
ID_HOLD_SECONDS = env_float("ID_HOLD_SECONDS", 0.80)
...
ACTIVITY_CONF = env_float("ACTIVITY_CONF", 0.45)
...
# -----------------------------------------------------------------------------
ID_AREA_MIN = env_int("ID_AREA_MIN", 600)
# Phone-screen photos can distort the apparent card ratio, so allow a
# wider range while rejecting boxes that cover most of the camera frame.
ID_RATIO_MIN = env_float("ID_RATIO_MIN", 0.70)
ID_RATIO_MAX = env_float("ID_RATIO_MAX", 3.60)
ID_AREA_MAX_RATIO = env_float("ID_AREA_MAX_RATIO", 0.45)
ID_MAX_WIDTH_RATIO = env_float("ID_MAX_WIDTH_RATIO", 0.92)
ID_MAX_HEIGHT_RATIO = env_float("ID_MAX_HEIGHT_RATIO", 0.88)
...
def valid_id_card_box(
    ...
    """Allow small cards shown on a phone while rejecting full-scene boxes."""
    if len(box) != 4:
        return False
...
def run_id_card_model(frame, model) -> tuple[int, list[dict[str, Any]]]:
    ...
    detections = remove_duplicate_boxes(detections, iou_threshold=0.50)
...
def run_inference(
    ...
    if "id_card" in models and (cycle == 1 or cycle % ID_MODEL_EVERY == 0):
        try:
            ...
            if new_id_count > 0:
                cache.id_count = new_id_count
                cache.id_dets = new_id_dets
                cache.last_id_time = now
            elif now - cache.last_id_time > ID_HOLD_SECONDS:
                cache.id_count = 0
                cache.id_dets = []
        except Exception as exc:
